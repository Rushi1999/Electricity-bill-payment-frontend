...
const SpringBootData = () => {
  ...
  const submitGetAllEmps = () => {
    axios
      .get(`http://localhost:8082/emp/getallemps`)
      .then((response) => {
  ...
  const submitGetEmpById = (evt) => {
    console.log(emp.eid);
    axios
      .get(`http://localhost:8082/emp/getempbyid/${emp.eid}`)
      .then((response) => {
  ...
  const submitAddEmp = (evt) => {
    evt.preventDefault();
    axios
      .post(`http://localhost:8082/emp/addemp`, newEmpObj)
      .then((response) => {
  ...
  return (
    <div className="container">
      ...
      <p className="text-primary">
        {emp.eid} {emp.firstName} {emp.salary}
      </p>
      <p>----------------</p>
      <div>
        ...
        {/* </form> */}
        <p>
          New Employee Data: {displayEmpObj.eid} {displayEmpObj.firstName}{" "}
          {displayEmpObj.salary}
        </p>
      </div>
      <p>----------------</p>
      <div>
                ...
                {empList.map((emp, k) => {
                  return (
                    <tr k={k} className="mdc-data-table__row" scope="row">
                      ...
                      <td className="mdc-data-table__cell">{emp.salary}</td>
                    </tr>
                  );
                })}
              </tbody>
